...
def Ticketfootball(request):
    if request.method=='POST':
        ...
        data.save()
    return render(request,'tickethub/football/index.html',{'tf':TicketFootball})
def Ticketplanes(request):
    if request.method=='POST':
        ...
        data.save()
    return render (request,'tickethub/plane/index.html')
def Ticketcinema(request):
    if request.method=='POST':
        ...
        a=request.POST.get("age")
        i=request.POST.get("id")
        data=TicketCinema(F_name=f,L_name=l,age=a,phone=p,id=i)
        if data.is_valid():
            data.save()
    # data.save()
    return render (request,'tickethub/cinema/index.html')